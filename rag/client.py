...
class RAGClient:
    ...
    def __init__(self, files, model = 'zephyr'):
        docs = load_pdf(files = files)
        self.retriever = create_parent_retriever(docs, self.embeddings_model)
    ...
    def retrieve_context(self, query: str):
        return retrieve_context(query, retriever = self.retriever, reranker_model = self.reranker_model)
    ...
    def generate(self, query : str) -> dict:
        ...
        return {
            ...
            'response': self.chain.invoke(
                {'context': context[0][0].page_content, 'question': query}
            )
        }
